...
const isServiceCredit = (r) => !r.reward_category || r.reward_category === 'service_credit';
...
const ServiceCreditsTab = ({ showToast, onCountChange, redemptionStats }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const p = (pendingRes.data.redemptions || []).filter(isServiceCredit);
      const f = (fulfilledRes.data.redemptions || []).filter(isServiceCredit);
      const d = (deniedRes.data.redemptions || []).filter(isServiceCredit);
  ...
  const handleApprove = async () => {
    try {
      ...
      showToast('Service credit marked as applied', 'success');
      setApproveItem(null);
      fetchData();
    } catch (err) {
  ...
  return (
    <Box>
      <Box sx={{ ...frostedCardSx, overflow: 'hidden', '&:hover': { transform: 'none' }, borderTop: '4px solid #D4956A' }}>
        ...
        <Box sx={{ px: 2.5, py: 1.5, borderBottom: '1px solid rgba(61,74,62,0.06)' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {redemptionStats?.scAmount > 0 && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <Box sx={{ textAlign: 'right' }}>
                  <Typography sx={{ fontFamily: '"Outfit", sans-serif', fontWeight: 700, fontSize: '18px', color: '#2D2D2D', lineHeight: 1.2 }}>
                    ${redemptionStats.scAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </Typography>
                ...
                {redemptionStats.totalRedemptionAmount > 0 && (
                  <Box sx={{
                    ...
                    <Typography sx={{ fontFamily: '"Outfit", sans-serif', fontWeight: 700, fontSize: '15px', color: '#D4956A', lineHeight: 1.2 }}>
                      {((redemptionStats.scAmount / redemptionStats.totalRedemptionAmount) * 100).toFixed(0)}%
                    </Typography>
          ...
          <>
            {isPending ? (
              ...
              <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {paged.length === 0 ? (
                  <Box sx={{ py: 6, textAlign: 'center' }}>
                    <Typography color="text.secondary">No pending service credits</Typography>
                  </Box>
                ) : paged.map((r) => (
                  <Box key={r.id} sx={{
                    p: 2.5,
                    ...
                    <Box sx={{ mb: 1.5 }}>
                      <Typography sx={{ fontWeight: 600, fontSize: '16px', color: '#2D2D2D', mb: 0.5 }}>
                        {r.reward_name || `${formatCurrency(r.credit_amount)} credit`}
                      </Typography>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                      ...
                      <Button size="small" variant="outlined" color="success" startIcon={<CheckCircleIcon sx={{ fontSize: '16px !important' }} />}
                        onClick={() => { setApproveItem(r); setCreditApplied(false); }}
                        sx={{ textTransform: 'none', fontSize: '13px', borderRadius: '8px' }}>Approve</Button>
              ...
              <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {grouped.length === 0 ? (
                  <Box sx={{ py: 6, textAlign: 'center' }}>
                    <Typography color="text.secondary">No {tab} service credits</Typography>
                  </Box>
                  ...
                  return (
                    <Box key={groupName} sx={{ borderRadius: '12px', border: '1px solid rgba(61,74,62,0.1)', overflow: 'hidden' }}>
                      ...
                      <Box onClick={() => toggleGroup(groupName)} sx={{
                        ...
                        <Typography sx={{ fontWeight: 600, fontSize: '14px', color: '#2D2D2D', flex: 1 }}>{groupName} Service Credit</Typography>
          ...
          {viewItem && (
            <Box sx={{ pt: 1 }}>
              <Box sx={{ background: 'rgba(61, 74, 62, 0.04)', borderRadius: '12px', p: 2, mb: 2 }}>
                ...
                <Typography variant="caption" sx={{ color: '#6B7A6D', textTransform: 'capitalize' }}>{viewItem.client_tier} tier</Typography>
              </Box>
              {[
                ['Reward', viewItem.reward_name || `${formatCurrency(viewItem.credit_amount)} credit`],
                ...
                ...(viewItem.voucher_code ? [['Voucher Code', viewItem.voucher_code]] : []),
              ].map(([label, value]) => (
      ...
      {/* Confirm Service Credit Applied Modal */}
      <Dialog open={!!approveItem} onClose={() => { setApproveItem(null); setCreditApplied(false); }} maxWidth="sm" fullWidth>
        <DialogTitle>Confirm Service Credit Applied</DialogTitle>
        <DialogContent>
          {approveItem && (
            <Box sx={{ pt: 1 }}>
              <Box sx={{ background: 'rgba(61, 74, 62, 0.04)', borderRadius: '12px', p: 2, mb: 2 }}>
                <Typography variant="body1" sx={{ fontWeight: 600, mb: 1 }}>{approveItem.client_name}</Typography>
                <Typography variant="body2" color="text.secondary">{approveItem.reward_name || `${formatCurrency(approveItem.credit_amount)} credit`}</Typography>
                <Box sx={{ display: 'flex', gap: 3, mt: 1.5 }}>
                  <Box><Typography variant="caption" color="text.secondary">Points Redeemed</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{(approveItem.points_redeemed || 0).toLocaleString()}</Typography></Box>
                  <Box><Typography variant="caption" color="text.secondary">Credit Value</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatCurrency(approveItem.credit_amount)}</Typography></Box>
                </Box>
              </Box>
              <FormControlLabel
                control={<Checkbox checked={creditApplied} onChange={(e) => setCreditApplied(e.target.checked)} color="success" />}
                label={<Typography variant="body2">I have applied a <strong>{formatCurrency(approveItem.credit_amount)}</strong> credit to <strong>{approveItem.client_name}</strong>'s next Care Package invoice.</Typography>}
                sx={{ mt: 1, alignItems: 'flex-start', '& .MuiCheckbox-root': { pt: 0.25 } }} />
            </Box>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => { setApproveItem(null); setCreditApplied(false); }}>Cancel</Button>
          <Button variant="contained" color="success" onClick={() => { handleApprove(); setCreditApplied(false); }} disabled={!creditApplied}>Mark as Applied</Button>
        </DialogActions>
      </Dialog>
...
export default ServiceCreditsTab;
