...
import { getRedemptions, fulfillRedemption, denyRedemption, getCardRequests, updateCardRequest } from '../../services/api';
import LoadingSpinner from '../../components/LoadingSpinner';
import StatusChip from '../../components/StatusChip';
import { frostedCardSx, TIER_DOT_COLORS } from '../../theme';

const isGiftCard = (r) => r.reward_category === 'gift_card';
...
const GiftCardsTab = ({ showToast, onCountChange, redemptionStats }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const pendingFiltered = (pendingRes.data.redemptions || []).filter(isGiftCard);
      const fulfilledFiltered = (fulfilledRes.data.redemptions || []).filter(isGiftCard);
      const deniedFiltered = (deniedRes.data.redemptions || []).filter(isGiftCard);

      // Merge approved card requests into pending
      let cardReqs = [];
      try {
        const crRes = await getCardRequests({ status: 'approved' });
        cardReqs = (crRes.data.requests || []).map(cr => ({
          ...cr, _type: 'card_request',
          reward_name: `Custom: ${cr.brand_name} ${cr.preferred_amount || ''}`.trim(),
          points_deducted: cr.points_deducted, credit_amount: cr.credit_amount,
          redeemed_at: cr.created_at, status: 'pending'
        }));
      } catch { /* card requests unavailable */ }

      const mergedPending = [...pendingFiltered, ...cardReqs];
      setAllData({ pending: mergedPending, fulfilled: fulfilledFiltered, denied: deniedFiltered });
      onCountChange(mergedPending.length);
    } catch (err) {
  ...
  const handleApprove = async () => {
    try {
      if (approveItem._type === 'card_request') {
        await updateCardRequest(approveItem.id, { delivery_code: approveDetails, admin_notes: approveNotes || undefined });
      } else {
        await fulfillRedemption(approveItem.id, { fulfillment_details: approveDetails, admin_notes: approveNotes });
      }
      showToast('Gift card approved and code sent', 'success');
  ...
  // Group items by brand for approved/denied views
  const grouped = useMemo(() => {
    if (tab === 'pending') return [];
    const groups = {};
    displayed.forEach(r => {
      const key = (r.reward_name || '').replace(/ Gift Card$/i, '') || formatCurrency(r.credit_amount);
  ...
  return (
    <Box>
      <Box sx={{ ...frostedCardSx, overflow: 'hidden', '&:hover': { transform: 'none' }, borderTop: '4px solid #D4956A' }}>
        ...
        <Box sx={{ px: 2.5, py: 1.5, borderBottom: '1px solid rgba(61,74,62,0.06)' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {redemptionStats?.gcAmount > 0 && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <Box sx={{ textAlign: 'right' }}>
                  <Typography sx={{ fontFamily: '"Outfit", sans-serif', fontWeight: 700, fontSize: '18px', color: '#2D2D2D', lineHeight: 1.2 }}>
                    ${redemptionStats.gcAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </Typography>
                ...
                {redemptionStats.totalRedemptionAmount > 0 && (
                  <Box sx={{
                    ...
                    <Typography sx={{ fontFamily: '"Outfit", sans-serif', fontWeight: 700, fontSize: '15px', color: '#D4956A', lineHeight: 1.2 }}>
                      {((redemptionStats.gcAmount / redemptionStats.totalRedemptionAmount) * 100).toFixed(0)}%
                    </Typography>
        ...
        {loading ? <LoadingSpinner /> : (
          <>
            {isPending ? (
              ...
              <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {paged.length === 0 ? (
                  <Box sx={{ py: 6, textAlign: 'center' }}>
                    <Typography color="text.secondary">No pending gift card redemptions</Typography>
                  </Box>
                ) : paged.map((r) => (
                  <Box key={r.id} sx={{
                    p: 2.5,
                    ...
                    {/* Top row: client + date + card request chip */}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Box>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            {r._type === 'card_request' && (
                              <Chip label="Card Request" size="small" sx={{ height: 20, fontSize: '10px', fontWeight: 600, backgroundColor: 'rgba(212,149,106,0.15)', color: '#A0673B' }} />
                            )}
                          </Box>
                    ...
                    <Box sx={{ mb: 1.5 }}>
                      <Typography sx={{ fontWeight: 600, fontSize: '16px', color: '#2D2D2D', mb: 0.5 }}>
                        {r.reward_name || `${formatCurrency(r.credit_amount)} gift card`}
                      </Typography>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.5, backgroundColor: 'rgba(61,74,62,0.06)', borderRadius: '6px', px: 1, py: 0.25 }}>
                          <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '13px', color: '#3D4A3E' }}>{(r.points_redeemed || r.points_deducted || 0).toLocaleString()}</Typography>
              ...
              /* -- Approved/Denied: Collapsible Dropdowns by Brand -- */
              <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {grouped.length === 0 ? (
                  <Box sx={{ py: 6, textAlign: 'center' }}>
                    <Typography color="text.secondary">No {tab} gift card redemptions</Typography>
                  </Box>
        ...
        <DialogContent>
          {viewItem && (
            <Box sx={{ pt: 1 }}>
              ...
              {[
                ['Reward', viewItem.reward_name || `${formatCurrency(viewItem.credit_amount)} gift card`],
      ...
      <Dialog open={!!approveItem} onClose={() => setApproveItem(null)} maxWidth="sm" fullWidth>
        <DialogTitle>Approve Gift Card</DialogTitle>
        <DialogContent>
          {approveItem && (
            <Box sx={{ pt: 1 }}>
              ...
              <TextField fullWidth required label="Code to Send to Client" value={approveDetails} onChange={(e) => setApproveDetails(e.target.value)} onPaste={(e) => e.preventDefault()} placeholder="e.g., Gift card code ABC-123" sx={{ mb: 2 }} />
